...
object WorkerMain extends App {
  ...
  val Array(masterHost, masterPortStr) = 
    masterAddr.split(":", 2) match { case Array(h, p) => Array(h, p) }
  ...
  if (WorkerDashboard.verbose) {
    WorkerDashboard.DashboardRenderer.start()
  }
  ...
  try {
    // ----------------- 3. register(init) -----------------
    if (WorkerDashboard.verbose) {
      WorkerDashboard.setStatus(WorkerDashboard.Registering)
    }
    ...
    if (WorkerDashboard.verbose) {
      val workerInfos = initReply.workerInfos.map(wi => (wi.ip, wi.port))
      WorkerDashboard.WorkerManager.initWorkers(workerInfos)
    }
    ...
    if (WorkerDashboard.verbose) {
      WorkerDashboard.setStatus(WorkerDashboard.Sampling)
    }
    ...
    val keyMessages: Seq[SampleKeyData.Key] = sampleKeys.map { k =>
      SampleKeyData.Key(keyDatum = ByteString.copyFrom(k.key))
    }
    ...
    val partitionRangesKey: List[(Key, Key)] = 
      partitionRangesMsg.partitionRanges.toList.map { pr => 
        (Key(pr.startKey.toByteArray), Key(pr.endKey.toByteArray))
      }

    // ----------------- 5. 파티션 함수 생성 -----------------
    val workerIpList: List[String] = currentWorkerInfos.map(_.ip).toList
    
    val partitionFunc: Key => String = Util.makePartition(partitionRangesKey, workerIpList)

    // ----------------- 6. 로컬 sort + partitioning -----------------
    if (WorkerDashboard.verbose) {
      WorkerDashboard.setStatus(WorkerDashboard.SortPartitioning)
    }
    ...
    if (WorkerDashboard.verbose) {
      WorkerDashboard.setStatus(WorkerDashboard.Shuffling)
    }
    ...
    if (WorkerDashboard.verbose) {
      WorkerDashboard.setStatus(WorkerDashboard.Merging)
    }
    ...
    if (WorkerDashboard.verbose) {
      WorkerDashboard.setStatus(WorkerDashboard.ShutdownWaiting)
    }
    ...
    while(!canShutdown) {
      ...
      if(reply.canShutdownWorkerServer) {
        ...
        if (WorkerDashboard.verbose) {
          WorkerDashboard.setStatus(WorkerDashboard.ShutdownRetrying)
        }
  ...
  private def fetchAllPartitionsForMe(
      ...
      initialVersion: Int): Future[List[String]] = {
    ...
    val perIpFutures: Seq[Future[List[String]]] = targetIps.map { targetIp =>
      Future {
        ...
        def refreshFromMaster(): Unit = {
          ...
          if (WorkerDashboard.verbose) {
            val workerInfos = curWorkers.map(wi => (wi.ip, wi.port))
            WorkerDashboard.WorkerManager.initWorkers(workerInfos)
          }
        }

        def findWorker(ip: String): Option[WorkerInfo] = curWorkers.find(_.ip == ip)
        ...
        def fetchOnceFromWorker(target: WorkerInfo): Either[Throwable, List[String]] = {
          ...
          try {
            ...
            if (WorkerDashboard.verbose) {
              WorkerDashboard.WorkerManager.initReceiving(targetIp)
            }
            ...
            blocking {
              for (partData <- it) {
                ...
                try {
                  ...
                  if (WorkerDashboard.verbose) {
                    WorkerDashboard.WorkerManager.incReceiving(targetIp)
                  }
                } finally {
                  out.close()
                }
              ...
              */
              e match {
                case sre: StatusRuntimeException => // grpc 에러만 따로 골라 잡기 히히
                  if (WorkerDashboard.verbose) {
                    WorkerDashboard.WorkerManager.setReceivingFailed(targetIp)
                  }
                case other =>
